...
# ---------------------------
# Dice loss function (for segmentation overlap)
# ---------------------------
...
# ---------------------------
# Load case names from text files
# ---------------------------
...
# ---------------------------
# General training loop (shared by both Stage 1 and 2)
# ---------------------------
...
# ---------------------------
# Dataset: actively selected labeled data (Dta)
# ---------------------------
...
# ---------------------------
# Dataset: pseudo-labeled samples excluding Dta (Dts)
# ---------------------------
class FilteredPseudoDataset(Dataset):
    def __init__(self, pseudo_dir, exclude_txt):
        with open(exclude_txt, 'r') as f:
            excluded = set(line.strip() for line in f)
        all_paths = sorted(glob(os.path.join(pseudo_dir, "*.h5")))
        self.paths = [p for p in all_paths if os.path.basename(p) not in excluded]
...
# ---------------------------
# Dataset: all pseudo-labeled samples (Stage 2)
# ---------------------------
class AllPseudoDataset(Dataset):
...
if __name__ == '__main__':
    ...
    # Stage 1: Train on Dta + Dts
    if args.epochs_stage1 > 0:
        logging.info(">>> Starting Stage 1 Training (Dta + Dts)")
        train_dataset_gt = GTOnlyDataset(args.data_root, args.active_txt, case_filter=train_cases)
        train_dataset_pseudo = FilteredPseudoDataset(args.pseudo_dir, args.active_txt)
        train_dataset_stage1 = ConcatDataset([train_dataset_gt, train_dataset_pseudo])
        train_loader1 = DataLoader(train_dataset_stage1, batch_size=args.batch_size, shuffle=True, num_workers=4)
        train_stage(model, train_loader1, args.data_root, val_cases, device, optimizer, args.epochs_stage1, "Stage1", args.save_dir)

    # Stage 2: Train on all pseudo-labeled samples (Dta + Dts)
    logging.info(">>> Starting Stage 2 Training (All pseudo-labeled slices)")
    model.load_state_dict(torch.load(os.path.join(args.save_dir, 'Stage1_best_model.pth')))
    train_dataset2 = AllPseudoDataset(args.pseudo_dir)
